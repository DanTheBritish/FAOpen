...
**Daedalus** is the modern rebuild of the core application for [FurAffinity](http://www.furaffinity.net/).
...
Typically, the URL pattern is as follows: `furaffinity.net/{module}/{controller}/{action}/{parameters}`, which corresponds to the following path: `/app/modules/{module}/controllers/{controller}Controller.php::{action}Action()`.
...
* **`fa` - FurAffinity Legacy:** Inherited functionality from the older FurAffinity system.
