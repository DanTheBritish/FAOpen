    ...
    public function init()
    {
        ...
        switch($this->settings['System_Mode'])
        {
            ...
            case self::SITE_MODE_READONLY:
                $flash->addMessage('Fur Affinity is in read-only mode.', 'warning');
                break;

            case self::SITE_MODE_FILE_READONLY:
                $flash->addMessage('Fur Affinity is in file read-only mode.', 'warning');
                break;

            case self::SITE_MODE_OFFLINE:
                die('FurAffinity is currently offline. Thank you for your patience!');
                break;
        }
